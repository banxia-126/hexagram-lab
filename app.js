...
// 拖动表体旋转。
// 窄屏下表壳整个不显示（见 style.css 末尾那条 @media），倾斜自然也不成立——
// 断点必须跟 CSS 那条**一模一样**，不然会出现「壳没了但还能拖」的错位。
const noShell = window.matchMedia('(max-width:520px),(max-height:620px)');

let spin = null, suppressClick = false;
$('watch').addEventListener('mousedown', (e) => {
  if (noShell.matches) return;              // 无壳：不响应倾斜
  ...
  b.innerHTML = `
    ...
    <div class="srow">
      ...
      <button class="pick" data-act="city">
        <span>${S.lonName}</span>
        <span class="pick-v">${S.trueSolar
          ? `经度修正 ${lonOffset(S.lon) >= 0 ? '+' : '−'}${Math.abs(lonOffset(S.lon)).toFixed(0)} 分　›`
          : '真太阳时已关，这项暂不生效　›'}</span>
      </button>
    </div>
    ...
    <div class="srow">
      ...
      <div class="srow-d">历史时区、地方性做法或别派算法用这里兜底。<b>这一项跟真太阳时开关无关</b>——关掉真太阳时它也照加。</div>
    </div>
    ...
    <div class="srow">
      ...
      <div class="srow-d">三钱法每爻等概率落在 6/7/8/9 上；大衍筮法取蓍草「十八变」的结果概率，<b>老阳明显偏多</b>。摇卦按这里选的法子，一爻一爻摇，共六次。</div>
    </div>
...
const TOPICS = {
  about: {
    t: '关于', h: `
      ...
      <h4>三套算法</h4>
      <p><b>六爻</b>　三钱摇卦 + 京房八宫装卦：纳甲、六亲、六神、世应。</p>
  ...
  liuyao: {
    t: '六爻 · 原理与出处', h: `
      ...
      <h4>两种筮法</h4>
      <p>三钱法三枚铜钱掷一次，6 / 7 / 8 / 9 四个值等概率出现；大衍筮法取蓍草<b>「十八变」
      的结果概率</b>，老阳(9)明显偏多、老阴(6)明显偏少。本工具<b>不模拟那十八变的过程</b>，
      直接按这套概率抽样 —— 过程是给眼睛看的，落定的值才是往下要用的。
      两者装卦的走法完全一样，在<b>设置</b>里切换。</p>
      <h4>装卦不用查表</h4>
      ...
      （旬空引擎已算出，盘面暂未显示）。可跑 <b>node test.mjs</b> 用乾宫八卦自校验。</p>
      <h4>原文</h4>
  ...
  meihua: {
    t: '梅花易数 · 原理与出处', h: `
      ...
      <h4>体用</h4>
      <p>动爻所在的那一卦为<b>用</b>，另一卦为<b>体</b>。体为己身，用为所应之事。</p>
      <h4>取象</h4>
      <p>体、用两卦各查一遍〈八卦萬物屬類〉，把卦还原成具体的东西：乾为天、为父、为金玉，
      坤为地、为母、为布帛，如此等等。原书那一列是从头顿号串到尾的，几十个词糊成一片，
      结果页改用「·」隔开。</p>
      <p>本工具<b>不出生克断语</b>。体克用、用生体那一套把六十四卦压成五个吉凶标签，
      读起来比卦本身还硬；取象才是把卦还原成具体事物的那一步，剩下的留给你自己看。</p>` + SRCNET,
  },
  settings: {
    t: '设置 · 分歧与出处', h: `
      <h4>为什么要给开关</h4>
      ...
      改完当下生效，存在本机浏览器里。</p>
      <p>下面两节讲<b>时间</b>上的两个分歧（晚子时、真太阳时）——它们影响全部三个板块。
      设置页还有几项，各自的道理写在别处：<b>经度</b>在设置页点开就是，<b>梅花起卦方式</b>
      见梅花易数的 i，<b>六爻筮法</b>见六爻的 i，<b>六宫五行</b>见小六壬的 i。</p>
      <h4>晚子时</h4>
